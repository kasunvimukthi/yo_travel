...
$sql = "SELECT * FROM `t_activities` WHERE `ID` = {$post_id}"; 
...
while($row = mysqli_fetch_assoc($result))
    ...
    <div class="image">
        <img src="imageView5.php?image_id=<?php echo $row["ID"]; ?>"width="100%" height="100%" />
...
<div class="row col-lg-12 bg-light ml-0">
  ...
  <div class="col-lg-6 bg-light">
    ...
    <div class="col-lg-12 border mt-4">
      ...
      <div class="row ml-0"><dt class="mr-1">Duration :</dt><p><?php echo $row["A_Duration"]; ?></p></div>
      <div class="row ml-0"><dt class="mr-1">Best Time :</dt><p><?php echo $row["A_Best_Time"]; ?></p></div>
    ...
    <div class="col-lg-12 mt-4">
        <div class="row1">
    <?php
              $sql = "SELECT * FROM `t_image`  WHERE `AC_ID` = ".$row["ID"];
  ...
  <div class="col-lg-12">
    <div class="map">
      <iframe src="<?php echo $row["A_Map"] ?>" width="100%" height="100%"></iframe>
    </div>
  </div>
...
<style>
  ...
  .my-card-body{
    ...
    <p class="h3">Related Activities</p>
    ...
    <?php
        ...
        $sql = "SELECT * FROM `p_activity` WHERE `Act_ID` = ".$row["ID"];
        $result1 = mysqli_query($conn, $sql);
        if(mysqli_num_rows($result1) > 0){
            while($row2 = mysqli_fetch_assoc($result1)) {

        $sql = "SELECT * FROM `p_activity` WHERE `P_ID` = ".$row2["P_ID"];
        $result2 = mysqli_query($conn, $sql);
        if(mysqli_num_rows($result2) > 0){
            while($row3 = mysqli_fetch_assoc($result2)) {

        $sql = "SELECT * FROM `t_activities` WHERE `ID` = ".$row3["Act_ID"];; 
        $result = mysqli_query($conn, $sql);
        if(mysqli_num_rows($result) > 0){
            while($row = mysqli_fetch_assoc($result)) {
                ?>
      ...
      <a href="activities.php?id=<?php echo $row['ID']; ?>">
